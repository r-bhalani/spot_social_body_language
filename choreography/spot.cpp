...
Spot::RobotCmdRespData Spot::velocityMoveTrajectory(double x, double y, double angular, google::protobuf::Timestamp endTime, gravAlignedFrame frame, bool relTime){
	RobotCommandResponse resp = _spotcontrol->velocityMoveTrajectory(x, y, angular, frame, relTime, endTime);
	Spot::RobotCmdRespData respData(resp.robot_command_id(),
		enumConvertRobotCommandStatus(resp.status()), 
		Spot::RobotCmdRespData::CommandType::VELOCITY_MOVE);
	return respData;
...
std::shared_ptr<CoreLayer::SpotBase> Spot::returnSpotBase() {
	return _spotcontrol->returnSpotBase();
}
...
//Spot Social Body Motions Method
/*Spot::ResetBodyMotionotCmdRespData Spot::setBodyMotion(Trajectory3D trajectory, bool gravityAlign, double x, double y, double angular, int64_t time, gravAlignedFrame frame) {
	BodyControlParams bodyParams;
    if(gravityAlign)
        bodyParams.set_rotation_setting(bosdyn::api::spot::BodyControlParams_RotationSetting_ROTATION_SETTING_ABSOLUTE);
    else
	...
	RobotCommandResponse resp = _spotcontrol-ParamsAndMove(mParams, x, y, angular, time, frame);
	Spot::RobotCmdRespData respData(resp.robot_command_id(),
		enumConvertRobotCommandStatus(resp.status()), 
		Spot::RobotCmdRespData::CommandType::VELOCITY_MOVE);
	return respData;
